...
import {
  FiCalendar, FiChevronDown, FiFilter, FiMapPin, FiSearch,
  FiSliders, FiTrendingUp, FiX,
} from 'react-icons/fi';
...
const CAT_LABELS = {
  HACKATHON:'Hackathon', WORKSHOP:'Workshop', TECHNICAL_SYMPOSIUM:'Symposium',
  CODING_COMPETITION:'Coding Comp', SEMINAR:'Seminar', PROJECT_EXHIBITION:'Project Expo',
  PLACEMENT_PREP:'Placement Prep', TECHNICAL_TRAINING:'Training', CULTURAL:'Cultural',
  SPORTS:'Sports', INTER_COLLEGE:'Inter-College', INTRA_COLLEGE:'Intra-College', OTHER:'Other',
};
const SORT = [
...
const initialFilters = {
  keyword:'', category:'', collegeName:'', departmentName:'', location:'', venueName:'',
  dateFrom:'', dateTo:'', eventType:'', priceMin:'', priceMax:'', freeOnly:false,
  paidOnly:false, interCollege:false, intraCollege:false, past:false,
  sortBy:'date_asc', availability:'',
};
...
export default function Events() {
  ...
  useEffect(() => {
    const t = setTimeout(() => setFilter('keyword', inputKw, false), 360);
  ...
  const clearAll = () => { setFilters(initialFilters); setInputKw(''); setPage(0); };
  ...
  return (
    <div className="min-h-screen bg-[#F0F4FF]">
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-3 lg:flex-row">
            <label className="relative flex-1" aria-label="Search events">
              <motion.span animate={{ x: inputKw ? 2 : 0, rotate: inputKw ? -8 : 0 }}
                className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-500">
                <FiSearch className="h-5 w-5" />
              </motion.span>
              <input value={inputKw} onChange={e => setInputKw(e.target.value)}
            ...
            <Segmented value={filters.sortBy} onChange={v => setFilter('sortBy', v)}
              options={[...SORT, { value:'alphabetical',label:'A-Z',icon:FiSliders }]} ariaLabel="Sort events" />

            <motion.button whileHover={{ scale:1.02 }} whileTap={{ scale:0.97 }}
              onClick={() => setFiltersOpen(v => !v)}
              className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-blue-100 bg-white px-4 text-sm font-bold text-blue-800 shadow-sm transition hover:bg-blue-50"
          ...
          <AnimatePresence>
            {activeBadges.length > 0 && (
              <motion.div initial={{ opacity:0,height:0 }} animate={{ opacity:1,height:'auto' }} exit={{ opacity:0,height:0 }}
                ...
                {activeBadges.map(b => (
                  <button key={b.key} onClick={() => setFilter(b.key, b.empty)}
                    className="inline-flex items-center gap-1.5 rounded-full border border-blue-100 bg-white px-3 py-1.5 text-xs font-semibold text-blue-800 shadow-sm transition hover:bg-blue-50">
                    {b.label}<FiX className="h-3.5 w-3.5" />
                  </button>
                ))}
                <button onClick={clearAll} className="rounded-full px-3 py-1.5 text-xs font-bold text-red-500 hover:bg-red-50">Clear all</button>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <main className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
        <AnimatePresence>
          {filtersOpen && (
            <motion.section initial={{ opacity:0,y:-10,height:0 }} animate={{ opacity:1,y:0,height:'auto' }}
              exit={{ opacity:0,y:-10,height:0 }} transition={{ duration:0.24 }}
              ...
              <div className="rounded-2xl border border-blue-100 bg-white p-5 shadow-card">
                <div className="mb-5 flex items-center justify-between gap-3">
                  ...
                  <button onClick={() => setFiltersOpen(false)} className="rounded-full p-2 text-slate-500 hover:bg-slate-100"><FiX /></button>
                </div>
                <div className="grid gap-5 lg:grid-cols-[1.35fr_1fr_1fr]">
                  <FilterGroup title="Category">
                    <div className="flex flex-wrap gap-2">
                      {CATS.map(cat => (
                        <Chip key={cat} active={filters.category === cat}
                  ...
                  <FilterGroup title="Price">
                    ...
                    <input type="range" min="0" max="10000" step="100" value={filters.priceMax || 0}
                      onChange={e => setFilter('priceMax', e.target.value)} className="w-full accent-blue-700" />
                  </FilterGroup>
        ...
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          {isFetching && (
            <span className="h-2 w-28 overflow-hidden rounded-full bg-blue-100">
              <motion.span animate={{ x:['-40%','120%'] }} transition={{ repeat:Infinity,duration:1.1 }}
                className="block h-full w-12 rounded-full bg-blue-600" />
            </span>
          )}
        </div>
          ...
          <>
            <motion.div layout className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
              {displayedEvents.map(ev => (
                <motion.div key={ev.id} layout initial={{ opacity:0,y:14 }} animate={{ opacity:1,y:0 }}
            ...
            {!displayedEvents.length && (
              <motion.div initial={{ opacity:0,y:10 }} animate={{ opacity:1,y:0 }}
                className="rounded-2xl border border-blue-100 bg-white py-20 text-center shadow-card">
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4">
      <button type="button" onClick={() => setOpen(!open)}
        className="mb-3 flex w-full items-center justify-between text-left" aria-expanded={open}>
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div initial={{ height:0,opacity:0 }} animate={{ height:'auto',opacity:1 }} exit={{ height:0,opacity:0 }}
            className="space-y-3 overflow-hidden">{children}</motion.div>
        )}
      </AnimatePresence>
    </div>
  );
...
function Chip({ active, onClick, children }) {
  return (
    <motion.button type="button" whileHover={{ scale:1.03 }} whileTap={{ scale:0.96 }} onClick={onClick}
      className={`rounded-full border px-3 py-1.5 text-xs font-bold transition ${active ? 'border-blue-700 bg-blue-700 text-white shadow-md' : 'border-blue-100 bg-white text-slate-600 hover:bg-blue-50'}`}
      aria-pressed={active}>
      {children}
    </motion.button>
  );
...
function Toggle({ active, onClick, children }) {
  return (
    <button type="button" onClick={onClick} className="flex items-center gap-2 text-sm text-slate-600">
  ...
  return (
    <label className="relative block">
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className={`h-10 w-full rounded-xl border border-blue-100 bg-white px-3 text-sm outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100 ${Icon ? 'pl-9' : ''}`}
        {...props} />
    </label>
  );
}
  ...
  return (
    <label className="block">
      ...
      <input type="date" value={value} onChange={e => onChange(e.target.value)}
        className="h-10 w-full rounded-xl border border-blue-100 bg-white px-3 text-sm outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100" />
    </label>
  );
}
...
function Segmented({ value, onChange, options, ariaLabel }) {
  return (
    <div className="flex h-12 max-w-full items-center gap-1 overflow-x-auto rounded-2xl border border-blue-100 bg-white p-1 shadow-sm"
      role="radiogroup" aria-label={ariaLabel}>
      {options.slice(0, 5).map(({ value: v, label, icon: Icon }) => (
        <button key={v} type="button" role="radio" aria-checked={value === v} onClick={() => onChange(v)}
          className={`relative flex h-9 shrink-0 items-center gap-1.5 rounded-xl px-3 text-xs font-bold transition ${value === v ? 'text-white' : 'text-slate-600 hover:bg-blue-50'}`}>
    ...
    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="h-72 animate-pulse rounded-2xl bg-white shadow-card">
          <div className="h-36 rounded-t-2xl bg-blue-100/70" />
...
function buildBadges(filters) {
  const labels = {
    ...
    collegeName: filters.collegeName, departmentName: filters.departmentName,
    location: filters.location, venueName: filters.venueName,
    ...
    freeOnly: filters.freeOnly ? 'Free' : '', paidOnly: filters.paidOnly ? 'Paid' : '',
  ...
  return Object.entries(labels).filter(([, label]) => Boolean(label))
